...
class BlockUserService {
  public async blockUser(userId: string, followerId: string): Promise<void> {
    UserModel.bulkWrite([
      {
        updateOne: {
          filter: { _id: new ObjectId(userId), blocked: { $ne: new mongoose.Types.ObjectId(followerId) } },
          update: {
            $push: {
      ...
      {
        updateOne: {
          filter: { _id: new ObjectId(followerId), blockedBy: { $ne: new mongoose.Types.ObjectId(userId) } },
          update: {
            $push: {
